...
async function runCLI(): Promise<void> {
  console.log("=== Status Monitor - Setup (TS) ===");
  ...
  const selected = (answers as any).translations_choice;
  const props = [
    `discord.bot.token=${(answers as any).discord_bot_token.trim()}`,
    `statuspage.name=${(answers as any).statuspage_name.trim()}`,
    `banner.live=${(answers as any).banner_live.trim()}`,
    `banner.history=${(answers as any).banner_history.trim()}`,
    `discord.channel.incident=${(answers as any).incident_channel.trim()}`,
    `discord.channel.dashboard=${(answers as any).dashboard_channel.trim()}`,
    `emoji.operational=${(answers as any).emoji_operational.trim()}`,
    `emoji.degraded_performance=${(answers as any).emoji_degraded_performance.trim()}`,
    `emoji.partial_outage=${(answers as any).emoji_partial_outage.trim()}`,
...
runCLI().catch(err => {
  console.error("Fehler im Setup (TS):", err);
  process.exit(1);
});
